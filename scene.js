...
import {compositionPose,assemblyPose} from './hero-choreography.js?v=R52';
...
function createScene(host,options){
 ...
 const handheld=innerWidth<700||matchMedia('(pointer:coarse)').matches;renderer.setPixelRatio(isRail?Math.min(devicePixelRatio,handheld?1.4:1.6):handheld?Math.min(devicePixelRatio,isHome?1.9:1.7):isHome?Math.min(Math.max(devicePixelRatio,2),2.5):Math.min(Math.max(devicePixelRatio,1.5),2.25));renderer.info.autoReset=false;renderer.transmissionResolutionScale=isRail?.55:handheld?.65:1;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1;renderer.shadowMap.enabled=!isRail;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.setClearColor(0xf0eee7,1);renderer.clear();renderer.domElement.style.background='#f0eee7';host.append(renderer.domElement);on(renderer.domElement,'webglcontextlost',e=>{e.preventDefault();cancelAnimationFrame(raf);raf=0;prepared=false});renderer.domElement.tabIndex=0;renderer.domElement.setAttribute('aria-label','Flacone 3D: trascina o usa le frecce per ruotarlo');renderer.domElement.style.touchAction='pan-y';
...
#include <colorspace_fragment>
 ...
 Promise.all([environmentReady,...needed.map(async(slug,i)=>{const gltf=await loader.loadAsync(`assets/models/${slug}.glb`);if(dead){release(gltf.scene);geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());textures.forEach(t=>t.dispose());return}const model=gltf.scene;model.position.y=-.615;// Flacone e tubicino escono da Blender con lo stesso materiale "Vetro ottico":
  // smaltirlo appena il flacone riceve il suo shader lasciava il tubicino con un
  // materiale morto, che si disegnava come una barra nera lampeggiante.
  const sharedUse=new Map();model.traverse(ob=>{if(ob.isMesh&&ob.material)sharedUse.set(ob.material,(sharedUse.get(ob.material)||0)+1)});
  const releaseMaterial=mat=>{const left=(sharedUse.get(mat)||1)-1;sharedUse.set(mat,left);if(left<=0)mat.dispose()};
  model.traverse(ob=>{if(!ob.isMesh)return;const name=ob.name.toLowerCase();ob.castShadow=name.includes('tappo');ob.receiveShadow=false;const mat=ob.material;mat.envMapIntensity=1.15;if(name.includes('flacone')||name.includes('tubicino')){ob.material=bottleGlass(interior,interiorSize);glassMeshes.push(ob);releaseMaterial(mat)}else if(name.includes('essenza')){ob.material=perfumeLiquid(slug,background,interiorSize);ob.visible=true;liquidMeshes.push(ob);releaseMaterial(mat);}else if(name.includes('etichetta')){labelMeshes.push(ob);addPaperEdge(ob)}finishSurface(ob.material,name,slug,renderer);if(isHome)finishMacroSurface(ob.material,name,renderer)});const pivot=new THREE.Group();pivot.add(model);pivot.userData.slug=slug;if(isHome){model.traverse(ob=>{if(!ob.isMesh)return;const n=ob.name.toLowerCase();if(n.includes('tappo')){pivot.userData.cap=ob;ob.userData.baseY=ob.position.y;ob.userData.baseSpin=ob.rotation.y}else if(n.includes('essenza')){pivot.userData.liquid=ob;ob.userData.baseY=ob.position.y;ob.geometry.computeBoundingBox();ob.userData.floor=ob.geometry.boundingBox.min.y;(pivot.userData.body||(pivot.userData.body=[])).push(ob)}else{(pivot.userData.body||(pivot.userData.body=[])).push(ob);ob.userData.baseY=ob.position.y}});}groups[i]=pivot;scene.add(pivot);let liquid,glass;model.traverse(o=>{if(o.isMesh&&o.name.toLowerCase().includes('essenza'))liquid=o;if(o.isMesh&&o.name.toLowerCase().includes('flacone'))glass=o});if(liquid&&glass)fluids.push({motion:createLiquidMotion(liquid,glass),pivot});const world=createFragranceWorld(slug,{lightSmoke:isRail});model.userData.fragranceSlug=slug;addStoneFragments(world,model);if(isRail){world.entranceMaterials=new Set();world.group.traverse(ob=>{if(ob.material&&!ob.material.isShaderMaterial){ob.material.transparent=true;ob.material.forceSinglePass=true;world.entranceMaterials.add(ob.material)}})}if(isHome){world.group.traverse(ob=>{if(ob.material?.uniforms?.field)ob.visible=false;if(ob.material?.uniforms?.densityBoost)ob.material.uniforms.densityBoost.value=3;if(ob.geometry?.isInstancedBufferGeometry)ob.visible=true;if(ob.isInstancedMesh)ob.count=Math.max(1,Math.ceil(ob.count*1.0));if(ob.isPoints)ob.geometry.setDrawRange(0,Math.ceil(ob.geometry.attributes.position.count*.40))})}world.group.traverse(ob=>{if(ob.material)for(const key of ['map','normalMap','roughnessMap','bumpMap'])if(ob.material[key])ob.material[key].anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy())});atmospheres[i]=world;scene.add(world.group);scene.add(world.setting.group);await world.setting.ready})]).then(async()=>{if(dead)return;applyResize();if(isRail){const visibility=atmospheres.map(w=>w.group.visible);atmospheres.forEach(w=>w.group.visible=true);renderer.compile(scene,camera);atmospheres.forEach((w,i)=>w.group.visible=visibility[i]);if(dead)return}prepared=true;if(!isHome&&!isRail){const note=document.createElement('span');note.className='fragrance-atmosphere-note';const title=document.createElement('span');title.className='fragrance-setting-title';title.textContent=atmospheres[0]?.setting.title||'';note.append(title,document.createTextNode(atmospheres[0]?.label||''));host.parentElement.append(note)}readScroll();if(!raf)raf=requestAnimationFrame(tick)}).catch(e=>{if(!dead){console.error('Unable to load 3D model',e);fallback()}});

 function tick(){raf=0;if(dead||!visible||document.hidden||!prepared)return;applyResize();const dt=Math.min(clock.getDelta(),.05),elapsed=clock.elapsedTime,mobile=host.clientWidth<700;progress=mix(progress,targetProgress,reduced.matches?1:1-Math.exp(-dt*9));if(Math.abs(progress-targetProgress)<.0001)progress=targetProgress;if(isRail&&host.dataset.ready==='true'&&railStart===null)railStart=performance.now();if(isRail&&railSelected&&railStart!==null)railElapsed+=dt;if(isRail&&railSelected&&!reduced.matches&&host.dataset.ready==='true')railAmbient+=dt;const railT=railStart===null?0:clamp(railElapsed/.85);railPhase=isCatalog?1:isRail?(railSelected?(reduced.matches?1:railT*railT*(3-2*railT)):0):1;railSpin=0;const p=progress,weights=[1];const activeIndex=weights.indexOf(Math.max(...weights));const bg=atmospheres[activeIndex]?.setting.color||new THREE.Color('#f0eee7');scene.background.copy(bg);camera.position.x=mix(camera.position.x,reduced.matches?0:pointer.x*.09,.06);camera.position.y=mix(camera.position.y,mobile?.05:.1+pointer.y*.07,.06);const closeup=0;camera.position.z=mobile?(isHome?3.8-closeup:3.0):(isHome?3.08-closeup:2.8);if(isRail)camera.position.set(0,.03,2.95);camera.lookAt(0,0,0);
 groups.forEach((g,i)=>{if(!g)return;if(!isHome){g.position.set(0,0,0);g.rotation.set(dragY,drag+railSpin,-.045);g.scale.setScalar(1);return}const pose=compositionPose(p,i,mobile,reduced.matches?0:elapsed,camera.aspect);g.userData.chapter=p;g.position.set(pose.x,pose.y,pose.z);g.rotation.set(dragY*.35+pose.pitch,drag*.35+pose.rotation,pose.tilt);g.scale.setScalar(pose.scale);g.visible=true;const asm=assemblyPose(p,mobile);g.userData.assembly=asm;const cap=g.userData.cap;if(cap){cap.position.y=cap.userData.baseY+asm.capY;cap.rotation.y=cap.userData.baseSpin+asm.capSpin}g.userData.body?.forEach(ob=>{ob.position.y=ob.userData.baseY+asm.bodyY});const liq=g.userData.liquid;if(liq){liq.scale.y=asm.fill;liq.position.y=liq.userData.baseY+asm.bodyY+liq.userData.floor*(1-asm.fill)}if(env)scene.environmentRotation.y=asm.light;
